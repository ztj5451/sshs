...
<div style="height: 588px;overflow-y: scroll;">
	<div class="loading" style="height: 650px;">
		<center>
			...
			<table>
				<tr>
					<td><div id="cpuchart"></div>
					</td>
					<td><div id="ramchart"></div>
					</td>
					<td><div id="jvmchart"></div>
					</td>
				</tr>
			</table>
			<br />
			<h1 style="background-color:#e8f3fd; width:99%;text-align: left;">磁盘信息</h1>
			<br />
...
<script type="text/javascript">
	var data = ${requestScope.message};//cpuInfos
	/*
	alert(warning);
	var data=warning.cpuInfos;
	alert(data);
	alert(warning.cpuUsage);
	*/
	
	var tbodyContent = "";
	tbodyContent = tbodyContent + "<tr>";
	tbodyContent = tbodyContent
			+ "<td>CPU</td><td style='padding-left:10px;text-align: left;'>使用率："
			+ data.cpuUsage + "%</td>";
	tbodyContent = tbodyContent
			+ "<td>使用率超出 <input class='inputclass' name='cpu' id='cpu' type='text' value='"+data.cpu+"'/> %,发送邮箱提示 <input type='button' value='更新' id='btnLoadData' class='input_btn_style1' onclick='modifySer(\"cpu\");'/></td><td rowspan='3'><input class='inputclass'style='width:180px;height:24px;' name='toEmail' id='toEmail' type='text' value='"+data.toEmail+"'/><input type='button' value='更新' id='btnLoadData' class='input_btn_style1'style='padding: 3px 11px;' onclick='modifySer(\"toEmail\");'/></td>";
	tbodyContent = tbodyContent + "</tr>";
	tbodyContent = tbodyContent + "<tr>";
	tbodyContent = tbodyContent
			+ "<td>服务器内存</td><td style='padding-left:10px;text-align: left;'>总内存："
			+ data.TotalMem + "<br/>空闲内存：" + data.FreeMem + "<br/>使用率："
			+ data.serverUsage + "%</td>";
	tbodyContent = tbodyContent
			+ "<td>使用率超出 <input class='inputclass' name='ram' id='ram' type='text' value='"+data.ram+"'/> %,发送邮箱提示 <input type='button' value='更新' id='btnLoadData' class='input_btn_style1' onclick='modifySer(\"ram\");'/></td>";
	tbodyContent = tbodyContent + "</tr>";
	tbodyContent = tbodyContent + "<tr>";
	tbodyContent = tbodyContent
			+ "<td>JVM内存</td><td style='padding-left:10px;text-align: left;'>JVM总内存："
			+ data.JvmTotalMem + "<br/>JVM空闲内存：" + data.JvmFreeMem
			+ "<br/>使用率：" + data.JvmUsage + "%</td>";
	tbodyContent = tbodyContent
			+ "<td>使用率超出 <input class='inputclass' name='jvm' id='jvm' type='text' value='"+data.jvm+"'/> %,发送邮箱提示 <input type='button' value='更新' id='btnLoadData' class='input_btn_style1' onclick='modifySer(\"jvm\");'/></td>";
	tbodyContent = tbodyContent + "</tr>";
	$('#tbody').html(tbodyContent);
	fchart("CPU使用情况", "%", "剩余", (100 - data.cpuUsage), "已经使用", data.cpuUsage,
			"cpu", "cpuchart");
	fchart("内存使用情况", "%", "剩余", (100 - data.serverUsage), "已经使用",
			data.serverUsage, "ram", "ramchart");
	fchart("JVM使用情况", "%", "剩余", (100 - data.JvmUsage), "已经使用", data.JvmUsage,
			"jvm", "jvmchart");
	...
	*/
	function fchart(name, pen, label1, value1, label2, value2, charId, divId) {
		...
		var chart = new FusionCharts(
				"${pageContext.request.contextPath}/FusionCharts/Pie3D.swf",
				charId, "350", "200");
